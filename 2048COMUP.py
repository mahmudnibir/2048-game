...
# Stack to store the grid state for undo functionality
undo_stack = []
...
def undo_move(grid, score):
    """Undo the last move by restoring the grid from the undo stack."""
    if undo_stack:
        last_state = undo_stack.pop()
        grid, score = last_state
    return grid, score
...
def main():
    ...
    while True:
        ...
        for event in pygame.event.get():  # Handle user input events
            if event.type == pygame.QUIT:
                ...
                if event.key in [pygame.K_LEFT, pygame.K_RIGHT, pygame.K_UP, pygame.K_DOWN]:
                    undo_stack.append((copy.deepcopy(grid), score))  # Save state for undo
                    ...
                    if moved:
                        add_random_tile(grid)  # Add a random tile after each move
                elif event.key == pygame.K_u:  # Undo the last move
                    grid, score = undo_move(grid, score)
                    add_random_tile(grid)  # Re-add a random tile after undoing
        ...
        if is_game_over(grid):  # Check if the game is over
            game_over = True
            save_highscore(score)  # Save the new high score if applicable
        clock.tick(15)  # Maintain frame rate
